...
import (
	...
	omni "github.com/voxgig-sdk/orbit-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.OrbitError); is {
				out := map[string]any{
					"name":    "OrbitError",
...
func omniCtx(arg any, client *sdk.OrbitSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.OrbitSDK); is {
			client = live
