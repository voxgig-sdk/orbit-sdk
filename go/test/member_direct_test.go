...
func TestMemberDirect(t *testing.T) {
	...
	t.Run("direct-load-member", func(t *testing.T) {
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-member", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
func memberDirectSetup(mockres any) *memberDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"ORBIT_TEST_MEMBER_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["ORBIT_TEST_MEMBER_ENTID"]; ok {
