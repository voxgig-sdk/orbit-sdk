...
func TestReportEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set ORBIT_TEST_REPORT_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		if reportRef01DataDt0Loaded == nil {
...
func reportBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("ORBIT_TEST_REPORT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ORBIT_TEST_REPORT_ENTID"])
