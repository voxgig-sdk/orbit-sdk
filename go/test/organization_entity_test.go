...
func TestOrganizationEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set ORBIT_TEST_ORGANIZATION_ENTID JSON to run live")
			return
		}
		client := setup.client
		...
		organizationRef01MarkdefUp0Name := "crm_uid"
...
func organizationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"organization01", "organization02", "organization03", "workspace_slug01"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("ORBIT_TEST_ORGANIZATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["ORBIT_TEST_ORGANIZATION_ENTID"])
