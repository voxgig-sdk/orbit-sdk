...
function isRunnable(code: string): boolean {
  return (
    /new\s+OrbitSDK\b/.test(code) ||
    /\bOrbitSDK\.test\b/.test(code) ||
    /\bclient\s*\./.test(code)
  )
}
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+OrbitSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/OrbitSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
