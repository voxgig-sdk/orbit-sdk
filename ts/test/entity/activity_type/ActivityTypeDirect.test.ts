...
describe('ActivityTypeDirect', async () => {
  ...
  test('direct-load-activity_type', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-activity_type', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['ORBIT_TEST_ACTIVITY_TYPE_ENTID']
