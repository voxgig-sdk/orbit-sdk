...
(0, node_test_1.describe)('ActivityTypeDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-activity_type', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-activity_type', setup.live))
            return;
        ...
        const result = await client.direct({
            path: '{workspace_slug}/activity_types',
            method: 'GET',
            params,
            query,
        });
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['ORBIT_TEST_ACTIVITY_TYPE_ENTID'];
