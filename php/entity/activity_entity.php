...
class ActivityEntity
{
    ...
    {
        if ($args) {
            $this->_data = OrbitHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = OrbitHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   SDK boundary; throws OrbitError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = OrbitHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *
     * @param ActivityCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed ActivityCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Activity|array The created Activity as an assoc-array at the
     *   SDK boundary; throws OrbitError on failure (item-5 convention).
     */
     ...
     *
     * @param ActivityUpdateData|array|null $reqdata Body data as an assoc-array;
     *   a typed ActivityUpdateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Activity|array The updated Activity as an assoc-array at the
     *   SDK boundary; throws OrbitError on failure (item-5 convention).
     */
    ...
    /**
     * Remove an Activity matching the given criteria.
     *
     * @param ActivityRemoveMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; ActivityRemoveMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Activity|array The removed Activity as an assoc-array at the
     *   SDK boundary; throws OrbitError on failure (item-5 convention).
     */
