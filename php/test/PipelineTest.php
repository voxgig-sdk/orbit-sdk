...
{
    private static function utility(): OrbitUtility
    {
        return new OrbitUtility();
    }
    ...
    private static function ctx(array $over = []): OrbitContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new OrbitContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new OrbitOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof OrbitError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new OrbitOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = OrbitMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new OrbitOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = OrbitMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new OrbitOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = OrbitMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OrbitSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = OrbitMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new OrbitResponse([]),
            'result' => new OrbitResult([]),
        ]);
        [, $err] = OrbitMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new OrbitSpec(['step' => 's']),
            'response' => new OrbitResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new OrbitResult([]),
        ]);
        [, $err] = OrbitMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new OrbitSpec(['step' => 's']),
            'response' => new OrbitResponse(self::resp(200, ['v' => 1])),
            'result' => new OrbitResult([]),
        ]);
        [, $err] = OrbitMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new OrbitSpec(['step' => 's']),
            'response' => new OrbitResponse(self::resp(200, ['v' => 2])),
            'result' => new OrbitResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        OrbitMakeResponse::call($ctx);
    ...
    {
        $preset = new OrbitResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new OrbitSpec([]),
            'response' => new OrbitResponse([]),
            'result' => new OrbitResult([]),
        ...
        [$got, $err] = OrbitMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new OrbitResult([])]);
        [, $err] = OrbitMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new OrbitSpec([]), 'result' => null]);
        [, $err] = OrbitMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OrbitSpec(['step' => 's']),
            'result' => new OrbitResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new OrbitOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OrbitMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OrbitSpec(['step' => 's']),
            'result' => new OrbitResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new OrbitOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OrbitMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OrbitResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new OrbitSpec([]),
            'result' => new OrbitResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = OrbitMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $utility = self::utility();
        $boom = new OrbitError('boom', 'boom');
        $utility->fetcher = function (OrbitContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OrbitSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = OrbitMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (OrbitContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = OrbitMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(OrbitResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (OrbitContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OrbitSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = OrbitMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new OrbitResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new OrbitSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = OrbitMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = OrbitMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = OrbitMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new OrbitResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, OrbitDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new OrbitResult(['ok' => false])]);
    ...
    {
        $ctx = self::ctx(['result' => new OrbitResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', OrbitMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new OrbitBaseFeature();
        $b = new OrbitBaseFeature();
        OrbitFeatureAdd::call($ctx, $a);
        OrbitFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): OrbitBaseFeature
    {
        $f = new OrbitBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    private static function auth_ctx(array $options, ?array $headers): OrbitContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new OrbitSpec(['headers' => $headers]),
        ]);
    }
    ...
    {
        $ctx = self::ctx([
            'response' => new OrbitResponse(['headers' => null]),
            'result' => new OrbitResult([]),
        ]);
        OrbitResultHeaders::call($ctx);
