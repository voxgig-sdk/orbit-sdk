...
class OrganizationEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set ORBIT_TEST_ORGANIZATION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $organization_ref01_markdef_up0_name = "crm_uid";
...
function organization_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["organization01", "organization02", "organization03", "workspace_slug01"] as $k) {
    ...
    $entid_env_raw = getenv("ORBIT_TEST_ORGANIZATION_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["ORBIT_TEST_ORGANIZATION_ENTID"]);
