...
describe("OrganizationEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set ORBIT_TEST_ORGANIZATION_ENTID JSON to run live")
      return
    end
    ...
    local organization_ref01_markdef_up0_name = "crm_uid"
...
function organization_basic_setup(extra)
  ...
  local idmap = vs.transform(
    { "organization01", "organization02", "organization03", "workspace_slug01" },
    {
      ["`$PACK`"] = { "", {
        ["`$KEY`"] = "`$COPY`",
  ...
  local entid_env_raw = os.getenv("ORBIT_TEST_ORGANIZATION_ENTID")
