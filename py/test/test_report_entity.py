    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set ORBIT_TEST_REPORT_ENTID JSON to run live")
        client = setup["client"]
        ...
        report_ref01_data_dt0_loaded = report_ref01_ent.load(report_ref01_match_dt0, None)
        assert report_ref01_data_dt0_loaded is not None
...
def _report_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("ORBIT_TEST_REPORT_ENTID"))
