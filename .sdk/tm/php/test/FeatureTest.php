...
class FtCtrl extends OrbitControl
{
...
class FtClient
{
    ...
    private OrbitUtility $utility;

    public function __construct(OrbitUtility $utility, array $options)
    {
    ...
    public function get_utility(): OrbitUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (OrbitContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility = new OrbitUtility();
        $this->utility = $utility;

        $utility->fetcher = $server ?? function (OrbitContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = OrbitConfig::make_feature($name);
    ...
    private static function buildUrl(OrbitSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof OrbitError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new OrbitResponse($fetched);
        }
    ...
    private function fail(OrbitContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (OrbitContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
