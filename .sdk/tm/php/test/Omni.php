...
//    only - a typed OrbitContext there would make every ctx
...
final class OrbitOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new OrbitContext($clean, null);
    ...
    public static function err_from_map($m): ?OrbitError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new OrbitError(is_string($code) ? $code : '', $msg);
    }
