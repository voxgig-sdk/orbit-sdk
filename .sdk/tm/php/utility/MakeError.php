...
class OrbitMakeError
{
    public static function call(?OrbitContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new OrbitContext([], null);
        }
        $op = $ctx->op ?? new OrbitOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new OrbitResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof OrbitError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new OrbitError('', $msg, $ctx);
