...
[MCP](https://modelcontextprotocol.io) server exposing the Orbit SDK as
two agent tools — `orbit_list` and `orbit_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/orbit-mcp)
make build
...
claude mcp add --scope user orbit \
  -- /absolute/path/to/orbit-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./orbit-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// orbit_list: first page of records
{ "entity": "member" }
{ "entity": "member", "query": { } }

// orbit_load: one record by id
{ "entity": "member", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/orbit-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user orbit \
     -- "$PWD"/dist/*/orbit-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `orbit_list` and `orbit_load` tools now appear
   in new sessions. Ask the agent to *"list member using orbit"*
   and it calls `orbit_list` with `{"entity":"member"}`.
...
| Tool | Args | Returns |
|------|------|---------|
| `orbit_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `orbit_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `ORBIT_APIKEY` | API key sent with every request. |
| `ORBIT_BASE` | Optional override of the API base URL. |
...
```sh
./orbit-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"orbit_load","arguments":{"entity":"member","query":{"id":1}}}}'
```
