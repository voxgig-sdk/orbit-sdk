// Command Orbit-mcp is an MCP server exposing the Orbit
// SDK as MCP tools.
//
// Two tools are registered:
//
//	Orbit_list  — list records of an entity
//	Orbit_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: ORBIT_APIKEY carries the API key and
	// ORBIT_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("ORBIT_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("ORBIT_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewOrbitSDK(opts)
