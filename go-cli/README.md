...
boru-driven command-line client **and** interactive REPL for the Orbit
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/orbit-cli)
make build
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/orbit-cli
   ```
...
Run with no arguments to open a REPL (prompt `orbit>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/orbit-cli`. |
...
(`./orbit-cli <expr>`) and interactively (the REPL), and expressions compose the
